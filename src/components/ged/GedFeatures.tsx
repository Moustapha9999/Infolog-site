...
export async function GedFeatures() {
  ...
  return (
    <section className="relative isolate overflow-hidden border-b border-ink/10 bg-paper-2/50 py-14 sm:py-16 lg:py-20">
      ...
      <Container className="relative max-w-7xl">
        <p className={cn(type.label, "text-copper")}>{ged.featuresLabel}</p>
        ...
        <ul className="mt-10 grid gap-px border border-ink/12 bg-ink/12 sm:grid-cols-2 lg:grid-cols-4">
          {ged.features.map((feature) => (
            <li
              key={feature}
              className="group bg-paper p-5 transition-colors duration-300 hover:bg-paper-2/80 sm:p-6"
            >
