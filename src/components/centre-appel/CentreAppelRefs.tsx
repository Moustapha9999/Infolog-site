  ...
  return (
    <section className="relative isolate overflow-hidden border-b border-ink/10 bg-paper py-14 sm:py-16 lg:py-20">
      <Container className="relative max-w-7xl">
        <p className={cn(type.label, "text-copper")}>{content.refsLabel}</p>
        <h2 className={cn(type.h2, "mt-3 text-ink")}>{content.refsTitle}</h2>

        <ul className="mt-8 flex flex-wrap gap-3">
          {content.refs.map((ref) => (
            <li
              key={ref.name}
              className="border border-ink/15 bg-paper-2/50 px-4 py-3"
            >
              <span className={cn(type.h3, "text-ink")}>{ref.name}</span>
              <span className={cn(type.bodyCard, "ml-2 text-mute")}>
                · {ref.note}
              </span>
            </li>
          ))}
        ...
        <div className="relative mt-14 overflow-hidden border border-ink/15 bg-ink px-6 py-10 sm:px-10 sm:py-12 lg:px-14 lg:py-14">
          <div
