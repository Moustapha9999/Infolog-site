        ...
        <div className="mt-10 max-w-none space-y-5 lg:max-w-[92%]">
            ...
            if ("highlights" in block) {
              return (
                <p key={index} className={cn(type.lead, "text-ink/80")}>
                  {block.before}{" "}
                  {block.highlights.map((name, i) => (
                    <span key={name}>
                      {i > 0 ? (
                        i === block.highlights.length - 1 ? (
                          " ou "
                        ) : (
                          ", "
                        )
                      ) : null}
                      <span className="font-medium text-plan">{name}</span>
                    </span>
                  ))}
                  {block.after}
                </p>
          ...
          <ul className="flex flex-wrap gap-2.5 pt-1">
            {content.tools.map((tool) => (
              <li
                key={tool}
                className="border border-ink/15 bg-paper px-3.5 py-2 font-mono text-xs tracking-wide text-plan-muted"
              >
                {tool}
              </li>
            ))}
